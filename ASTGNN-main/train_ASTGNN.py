...
# Modify to use CPU if CUDA is unavailable or explicitly requested
USE_CUDA = torch.cuda.is_available() and args.cuda != 'cpu'
...
def compute_and_write_metrics(epoch, data_target_tensor, prediction, csv_filename):
    """
    Compute and write MAE, RMSE, and MAPE metrics to a CSV file for a given epoch.

    :param epoch: Current epoch (int).
    :param data_target_tensor: Ground truth tensor (numpy array).
    :param prediction: Predicted tensor (numpy array).
    :param csv_filename: Path to the CSV file where metrics will be written (str).
    """
    ...
    # Reshape tensors for metric calculation
    data_target_tensor = data_target_tensor.reshape(-1, 1)
    prediction = prediction.reshape(-1, 1)

    # Normalize the data using Min-Max Scaling
    scaler = MinMaxScaler()
    data_target_tensor = scaler.fit_transform(data_target_tensor)
    prediction = scaler.transform(prediction)

    # Calculate metrics
    mae = mean_absolute_error(data_target_tensor, prediction) * 100
    rmse = np.sqrt(mean_squared_error(data_target_tensor, prediction)) * 100
    # Calculate MAPE using the masked_mape_np function
    mape = masked_mape_np(data_target_tensor, prediction, null_val=0, min_val=1e-2) / 100
    ...
    with open(csv_filename, 'a', newline='') as csvfile:
        ...
        # Write the metrics for this epoch
        csvwriter.writerow([epoch, mae, rmse, mape])
...
def train_main():
    ...
    # Define the CSV file path for metrics
    metrics_csv_path = os.path.join(params_path, "metrics.csv")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Write metrics at specific epochs
        if epoch in [80, 100, 150, 200, 300, 400, 500, 600, 700, 800]:
            # Generate predictions (Replace this with actual predictions from your model)
            predictions = np.random.rand(*val_target_tensor.shape)  # Placeholder for predictions
            compute_and_write_metrics(epoch, val_target_tensor.cpu().numpy(), predictions, metrics_csv_path)
